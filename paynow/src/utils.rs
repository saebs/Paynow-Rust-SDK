...
pub fn is_valid_hash(
    ...
    let two = hash_local.unwrap().clone();
    let one = hash.as_str();
    assert_eq!(one, two);
    Ok((hash.as_str() == two, hash))
}
...
mod tests {
    ...
    fn validates_hash() {
        ...
        let hash_o = "750DD0B0DF374678707BB5AF915AF81C228B9058AD57BB7120569EC68BBB9C2EFC1B26C6375D2BC562AC909B3CD6B2AF1D42E1A5E479FFAC8F4FB3FDCE71DF4D";
        assert_eq!(
            is_valid_hash(response, integration_key).unwrap(),
            (true, hash_o.to_string())
        );
    }
}
